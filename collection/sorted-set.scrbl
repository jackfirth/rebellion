...
A @deftech{sorted set} is a collection of unique values sorted according to a @tech{comparator}.
Sorted sets are @tech/reference{sequences}, and when used as a sequence the set's elements are
iterated in ascending order. Sorted sets are either mutable or immutable. Immutable sorted sets are
@racket[equal?] when they contain the same elements and their comparators are equal. Mutable sorted
sets are @racket[equal?] when their comparators are equal and they will @emph{always} contain the
same elements. That is, if two mutable sorted sets are equal, then adding and removing elements from
one set will cause them to be added and removed in the other.


@defproc[(sorted-set? [v any/c]) boolean?]{
 A predicate for @tech{sorted sets}.}


@defproc[(immutable-sorted-set? [v any/c]) boolean?]{
 A predicate for immutable @tech{sorted sets}.}


@defproc[(mutable-sorted-set? [v any/c]) boolean?]{
 A predicate for mutable @tech{sorted sets}.}
