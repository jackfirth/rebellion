...
(define-syntax-class enum-id
  ...
  (pattern binding
    ...
    #:with selector #'(enum-descriptor-selector descriptor)
    #:with discriminator #'(enum-descriptor-discriminator descriptor)
    #:with (constant ...)
    (for/list ([n (in-range (enum-type-size (attribute type)))])
      #`(selector '#,n))
    #:with (constant-name ...)
