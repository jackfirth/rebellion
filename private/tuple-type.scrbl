...
implementation of a tuple type is obtained via @racket[make-tuple-type], which
generates a new implementation of a tuple type and returns a @deftech{tuple type
 descriptor} object. Descriptors provide functions that implement the type and
a predicate that identifies the implementation's values. Descriptors are
generative; calling @racket[make-tuple-type] with the same tuple type will make
multiple distinct implementations of that type.
...
@deftogether[[
 ...
 of a descriptor completes when @racket[make-tuple-type] returns.}
...
@defproc[
 (make-tuple-type [type tuple-type?]
...
@defproc[
 ...
 debugging purposes. See @racket[make-tuple-type] for usage examples.}
...
@defproc[
 ...
 called by @racket[make-tuple-type] when no @racket[_prop-maker] argument is
 supplied.}
