...
(module+ test
  ...
  (test-case "sorting"
    ...
    (check-equal? (transduce (list 1 2 3) (sorting) #:into into-list)
                  (list 1 2 3))
    (check-equal? (transduce (list (ruby 1) (sapphire 1) (emerald 2) (topaz 2))
                             (sorting #:key gemstone-weight)
                             #:into into-list)
                  (list (ruby 1) (sapphire 1) (emerald 2) (topaz 2)))
    (check-equal? (transduce (list 3 2 1) (sorting) #:into into-list)
                  (list 1 2 3))
    (check-equal? (transduce (list 2 1 3) (sorting) #:into into-list)
                  (list 1 2 3))
