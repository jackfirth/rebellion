...
(define (cut<=> base-comparator)
  (define/guard (cmp left right)
    (guard (and (equal? left bottom-cut) (equal? right bottom-cut)) then equivalent)
    (guard (equal? left bottom-cut) then lesser)
    (guard (equal? right bottom-cut) then greater)
    (guard (and (equal? left top-cut) (equal? right top-cut)) then equivalent)
    (guard (equal? left top-cut) then greater)
    (guard (equal? right top-cut) then lesser)
    (define result
      (compare
       base-comparator
